...
bauturiDb.onSnapshot(extractProducts);
...
function renderObj(products) {
  ...
  for (let item in products) {
    for (let bauturi of products[item]) {
      html += `
      <li class="cardLi" data-item-id="${bauturi.id}">
          <div class="card">
            <img src="${bauturi.img}" class="card-img">
            <div class="card-overlay">
              <div class="card-header">
                <div class="card-header-text">
                  <h3 class="card-title">${bauturi.nume}</h3>
                  <span class="product-price">${bauturi.pret} MDL</span>
                  <span id="del" class="delete">
                  ${
                    user
                      ? `<i class="fas fa-trash" id="bin" onclick="sterge('${bauturi.id}')"></i>`
                      : ''
                  }
                  </span>
                  <span class="add-btn">
